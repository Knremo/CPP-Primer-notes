...
```c++
size_t cnt = 0;
for (auto &row : ia) // int (&row)[4] : ia
    for (auto &col : row){ // int &col : row
        col = cnt;
        ++cnt;
    }
```
...
```c++
int ia[3][4] = {};
for (auto p = ia; p != ia + 3; ++p) { // int (*p)[4] = ia
    for (auto q = *p; q != *p + 4; ++q) //指针p指向一个含有四个整数的数组，解引用之后才是该数组 int *q = *p
        cout << *q << ' ';
    cout << endl;
}
// ==
